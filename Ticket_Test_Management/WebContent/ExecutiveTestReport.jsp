...
<option>----select-----</option>
							...
							<br> <br> Requirement Name:&nbsp;&nbsp;&nbsp; 
							
							<select id="hosting-plan" name="requirementname">
<option>----select-----</option>
								<%
								
								ConnectionSteps steps1 = new ConnectionSteps();
								Connection conn1 = steps1.connection();
								PreparedStatement pstmt1 = conn1.prepareStatement("select * from tickettable");
								ResultSet rs1 = pstmt.executeQuery();
								while (rs1.next()) {
							%>
								
								<option value=<%=rs1.getString("requirementname")%>><%=rs1.getString("requirementname")%></option>
								<%}%>
								</select> 
							...
							<br> <br> Module Name:&emsp;&emsp;&emsp;&nbsp; 
							<select id="hosting-plan" name="modulename">
                        <option>----select-----</option>
								<%
								
								ConnectionSteps steps2 = new ConnectionSteps();
								Connection conn2 = steps.connection();
								PreparedStatement pstmt2 = conn2.prepareStatement("select * from tickettable");
								ResultSet rs2 = pstmt.executeQuery();
								while (rs2.next()) {
							%>
								
								<option value=<%=rs2.getString("modulename")%>><%=rs2.getString("modulename")%></option>
								<%}%>
								</select>
							
								&emsp;&emsp;&emsp;&nbsp; <input type="submit" name="submit"
								value="Submit" /><br> <br>
					...
					<br />
					<%
						if ((user.getProjectName().equals("empty")) && (user.getRequirementName().equals("empty"))
								&& (user.getModuleName().equals("empty"))) {
					%>

					<form method="post" id="insert_form">
						<div class="table-repsonsive">
							<span id="error"></span> <input type="button"
								onClick="deleteSelectedRows()" value="DeleteRows"></input> <br>
							<br>
					...
					<form method="post" id="insert_form">
						<div class="table-repsonsive">
							...
							<form action="TestReportDeleteRows.jsp">

								<button type="submit" name="test" onClick="deleteSelectedRows()"
									value="DeleteRows">DeleteRows</button>
								<br> <br>
								
								<b>ProjectName:::</b>
								<%=user.getProjectName()%>
								&emsp;&emsp;<b>RequirementName:::</b>
								<%=user.getRequirementName() %>
								...
								<table class="table table-bordered" id="item_table">
									...
									<%
										ConnectionSteps steps3 = new ConnectionSteps();
											Connection conn3 = steps3.connection();
											PreparedStatement pstmt3 = conn3.prepareStatement(
													"select * from testreporttable where projectname=? and requirementname=? and modulename=?");
											pstmt3.setString(1, user.getProjectName());
											pstmt3.setString(2, user.getRequirementName());
											pstmt3.setString(3, user.getModuleName());
									...
									<tr>
										<td contentedible="false"><input type="checkbox"
											name="test" value=<%=rs3.getString("testcaseid")%>></td>
										<td><div contenteditable class="update"
												data-id=<%=rs3.getInt("id")%> data-column="testcaseid"><%=rs3.getString("testcaseid")%></div></td>
										<td>
											<div contenteditable class="update"
												data-id=<%=rs3.getInt("id")%> data-column="testdescription"><%=rs3.getString("testdescription")%></div>
										</td>
										<td>
											<div contenteditable class="update"
												data-id=<%=rs3.getInt("id")%> data-column="precondition"><%=rs3.getString("precondition")%></div>
										</td>
										<td>
											<div contenteditable class="update"
												data-id=<%=rs3.getInt("id")%> data-column="testdesign"><%=rs3.getString("testdesign")%></div>
										</td>
										<td>
											<div contenteditable class="update"
												data-id=<%=rs3.getInt("id")%> data-column="expectedresult"><%=rs3.getString("expectedresult")%></div>
										</td>
										<td>
											<div contenteditable="false" class="update"
												data-id=<%=rs3.getInt("id")%> data-column="actualresult"><%=rs3.getString("actualresult")%></div>
										</td>
										<td>
											<div contenteditable="false" class="update"
												data-id=<%=rs3.getInt("id")%> data-column="status"><%=rs3.getString("status")%></div>
										</td>
										<td>
											<div contenteditable="false" class="update"
												data-id=<%=rs3.getInt("id")%> data-column="comments"><%=rs3.getString("comments")%></div>
										</td>
						...
						$(document)
								.on(
										...
										function() {
											...
											html += '<td ><input type="text" name="testcaseid[]" class="form-control item_name" value="TC-0"+i /></td>';
